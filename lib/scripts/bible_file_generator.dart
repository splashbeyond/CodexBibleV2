import 'dart:io';
import 'bible_text_parser.dart';

void main() async {
  final generator = BibleFileGenerator();
  await generator.generateFiles();
}

class BibleFileGenerator {
  ...
  final Map<String, Map<String, dynamic>> books = {
    'Genesis': {'number': '002', 'abbr': 'GEN', 'chapters': 50, 'testament': 'Old_Testament'},
    'Exodus': {'number': '003', 'abbr': 'EXO', 'chapters': 40, 'testament': 'Old_Testament'},
    'Leviticus': {'number': '004', 'abbr': 'LEV', 'chapters': 27, 'testament': 'Old_Testament'},
    'Numbers': {'number': '005', 'abbr': 'NUM', 'chapters': 36, 'testament': 'Old_Testament'},
    'Deuteronomy': {'number': '006', 'abbr': 'DEU', 'chapters': 34, 'testament': 'Old_Testament'},
    // Add all books here with their details
  };

  Future<void> generateFiles() async {
    for (final book in allBooks) {
      print('Generating files for ${book.name}...');
      
      // Process the entire book first
      final bookPath = '$sourceDir/${book.name.toLowerCase().replaceAll(' ', '_')}.txt';
      ...
      for (int chapter = 1; chapter <= book.chapters; chapter++) {
        ...
        final dirPath = '$baseDir/${book.testament}';
        ...
        if (!await file.exists() || (await file.readAsString()).trim().isEmpty) {
          ...
          if (chapters.containsKey(chapter)) {
            ...
            print('Added ${verses.length} verses to $fileName');
          } else {
            print('No verses found for ${book.name} chapter $chapter');
          }
        } else {
          print('File already exists and has content: $fileName');
        }
      }
    }
...
// Add a class to store book data
class BibleBook {
  final String name;
  final String number;
